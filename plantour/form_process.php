...
$address_error= $phone_error =  $zipcode_error =$foto_error=$ttype_error=$visit_error=$datefirst_error=$datelast_error= $cardnum_error=$seccode_error=$expdate_error=$cholder_error=$accept_error="";

$errors=false;

//variablat pervlerat e te dhenave
$name = $lname = $phone = $address = $zipcode =$transport=$datefirst=$datelast=$visit= $cardnum= $seccode= $expdate= $cholder =$seccodemd5=$price="";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (isset($_POST["visit"]) && $_POST["visit"]!="") {
		$visit= test_input($_POST["visit"]);
		$errors=true;
	}
	else{
		$visit_error="Select one of the options";
		$errors=false;
	}

	if (isset($_POST["ttype"]) && $_POST["ttype"]!="") {
		$transport= test_input($_POST["ttype"]);
		$errors=true;
	}
	else{
		$ttype_error="Select one of the options";
		$errors=false;
	}
	...
	if (empty($_POST["datefirst"])) {
		$datefirst_error = "First date  is required";
		$errors=false;
	} else {
		$datefirst = test_input($_POST["datefirst"]);
		$errors=true;
	}
	...
	if (empty($_POST["cardnumber"])) {
		...
		if ((!preg_match($cardtype['visa'],$cardnum)) &&
			(!preg_match($cardtype['mastercard'],
				$cardnum)) 
			&& (!preg_match($cardtype['discover'],$cardnum)) && (!preg_match($cardtype['amex'],$cardnum))){
			$cardnum="";
...
if ($errors) {
	...
	$sql1 = "INSERT INTO visitdetails  (ttype, datefirst, datelast, visit) VALUES ('$transport', '$datefirst','$datelast','$visit')";
